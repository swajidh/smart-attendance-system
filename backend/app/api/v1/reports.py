...
from app.api.dependencies import get_db_session, require_reports_read, require_export_reports, get_current_user
from app.models.user import User
from app.services import report_service, export_service

router = APIRouter(prefix="/reports", tags=["reports"])

_READ = [Depends(require_reports_read)]
_EXPORT = [Depends(require_export_reports)]


# ── 5.3 Summary ───────────────────────────────────────────────────────────────

@router.get("/attendance", dependencies=_READ)
...
@router.get("/attendance/student/{student_id}", dependencies=_READ)
...
@router.get("/at-risk", dependencies=_READ)
async def at_risk_students(
    ...
    return await report_service.get_at_risk_students(
        db, threshold, department, student_ids=scope
    )


@router.get("/trends", dependencies=_READ)
...
@router.get("/last-seen", dependencies=_READ)
async def last_seen(
...
async def dashboard_summary(
    ...
    return await report_service.get_dashboard_summary(db, student_ids=scope)


# ── 7.4 Correlation routes ────────────────────────────────────────────────────

@router.get("/correlation/student/{student_id}", dependencies=_READ)
...
@router.get("/correlation/batch", dependencies=_READ)
async def batch_correlation(
    ...
    from app.services import batch_service
    scope = await batch_service.scope_for_user(db, current_user, batch_id)
    return await get_batch_correlation(
        db, department, limit, batch_id=batch_id, student_ids=scope
    )


# ── 5.3 Exports ───────────────────────────────────────────────────────────────

@router.get("/export/csv", dependencies=_EXPORT)
...
@router.get("/export/pdf", dependencies=_EXPORT)
